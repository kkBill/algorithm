...
全局变量maxSum用来记录当前根节点是否纳入为最终的路径中，因此我们这样考虑`maxSum = Math.max(maxSum, root.val + left + right);`；但是对于上一层，我们只能选择以当前根节点的一个分支，因此写成`Math.max(left, right) + root.val;`。

另外，需要特别注意的是——我们在计算左右分支时，分支计算出来的值有可能是负的，如果是负的，我们就不选择这一条分支，因此，需要写成`int left = Math.max(dfs(root.left), 0);`，而不是直接写成`int left = dfs(root.left);`。
...
public class Codec {
    ...
    public String serialize(TreeNode root) {
        ...
        return sb.substring(0, sb.length()-1-lastLevelCnt*5) + "]";
    }
